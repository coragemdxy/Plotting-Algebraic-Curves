...
\begin{proof}
    ...
    Finally, we will prove that \(\forall a \in A, \exists a^{-1} \in A\). We know that \(a \in A\), there exists a rational polynomial \(P = \sum_{i=0}^{n}(c_{i}X^{i})\), such that \(P(a) = 0\). We can construct a new polynomial \(Q = \sum_{i=0}^{n}(c_{i}X^{n-i})\), \(Q(a^{-1}) = ({(a^{-1})}^{n})*\sum_{i=0}^{n}(c^{i}a^{i}) = ({(a^{-1})}^{n})* P(a) = 0\), so \(a^{-1} \in A\).
